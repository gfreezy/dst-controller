-- Enhanced Controller - Search & Cook button for the in-game cookbook
...
function CookbookHook.Install()
    ...
    G.AddClassPostConstruct("widgets/redux/cookbookpage_crockpot", function(page)
        local TEMPLATES = require("widgets/redux/templates")
        local cooking = require("cooking")
        local old_PopulateRecipeDetailPanel = page.PopulateRecipeDetailPanel
        ...
        page.PopulateRecipeDetailPanel = function(self, data)
            ...
            local cooker_prefabs = Planner.ResolveCookerPrefabs(cooking,
                data and data.prefab)
        ...
        -- The original constructor populated the selected detail before this
        -- post-construction hook was attached, so rebuild that one panel once.
        local cookbook = G.TheCookbook
        local selected_index = cookbook ~= nil and cookbook.selected ~= nil and
            cookbook.selected[page.category] or 1
        local selected = page.all_recipes and page.all_recipes[selected_index]
        if selected ~= nil and page.details_root ~= nil then
            page.details_root:KillAllChildren()
            page.details_root:AddChild(page:PopulateRecipeDetailPanel(selected))
        end
        ApplyFocus(page)
    end)
end
