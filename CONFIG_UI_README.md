...
在游戏中按下 **Ctrl + K**，或同时按下手柄 **LB + RB + Y**，即可打开配置界面。
...
- **检查**: 检查物品或生物
- **检查自己**: 检查玩家自身
- **使用光标物品（自身/场景）**: 使用虚拟光标当前拿起的物品
- **保存手持物品**: 保存当前手持的物品
- **恢复手持物品**: 恢复之前保存的物品
- **切换头部装备**: 切换头部槽位的装备
- **切换手部装备**: 切换手部槽位的装备
- **切换身体装备**: 切换身体槽位的装备
- **启用/关闭虚拟光标**: 显式切换虚拟光标模式
...
- **使用装备**: 使用指定装备
- **延迟**: 在动作序列中等待指定时间
- **对场景使用物品**: 对目标使用指定物品（需要参数：物品名称）
- **对自己使用物品**: 对自己使用物品（需要参数：物品名称）
- **制作物品**: 自动搜索附近材料/容器并制作指定配方；后续动作会等待完成
- **触发按键**: 触发指定键盘按键

### Mod 设置

“Mod 设置”标签页提供攻击/交互角度、空气攻击、虚拟光标速度与显示、世界/UI 磁吸范围和优先级，以及 ActionQueue RB3 手柄适配。虚拟光标不再叠加额外死区。
...
1. 配置完成后，点击"应用"按钮
2. 配置会立即生效，无需重启游戏
3. 配置自动保存到 `client_save/enhanced_controller_config.json`
...
scripts/
└── dst-controller/
    ├── screens/taskconfig-screen.lua  # 配置界面 UI 组件
    ├── hooks/taskconfig-hook.lua       # 快捷键监听和界面管理
    └── utils/config_manager.lua        # 配置加载和持久化
```

### 配置流程

1. **加载**: `ConfigManager` 从客户端存档加载配置，并补齐新版本默认字段
2. **运行时更新**: 用户在界面中修改配置，保存在内存中
3. **应用**: 保存 JSON 配置并更新运行时配置
...
3. 对场景使用物品(lighter) - 开始使用打火机

**松开动作序列**:
1. 对场景使用物品(lighter) - 停止使用打火机
2. 恢复手持物品 - 恢复之前的物品
...
1. **物品名称**: 使用"装备物品"等需要参数的动作时，物品名称必须是游戏内部的 prefab 名称（如 lighter、torch），而不是显示名称。

2. **自动建造中断**: 手动控制、死亡、动作失败或材料变化都会停止自动建造；已经发生的取材和制作不会回滚。

3. **ActionQueue**: 手柄适配只有在安装 ActionQueue RB3、启用对应设置并打开虚拟光标时生效。
...
║  ─────────────────────────────────────────────────────   ║
║  3.  ↑  use_item_on_scene(lighter)  [编辑]  [删除]       ║
║                                                           ║
║                    [ + 添加动作 ]                         ║
║                                                           ║
╠═══════════════════════════════════════════════════════════╣
...
║                      【松开动作列表】                      ║
║                                                           ║
║  1.  ↓  use_item_on_scene(lighter)  [编辑]  [删除]       ║
║  ─────────────────────────────────────────────────────   ║
